...
| G-scope supports (`app-gscope-support.js`) | sub-region | the marker scan's skipList, `nsoMaskFaceList(m)` shifted into the world frame; refused by name when the piece is painted and turned. Generating adds a piece (none); removing or changing one acts on the whole support piece | header comment + `docs/GCODE-SUPPORTS.md` |
...
3. Every selection change (`state.selectedIndex` / `state.editId`, however
   assigned) as its own event, logged before the operation that follows it;
   every operation as `preview` / `discard` / `commit` / `undo`, carrying the
   selection it acted on; build-volume warnings, which `app-fit.js` now
   routes through `setStatus` when they change. The brush announces its
   preview's two ends as a `nso:op` DOM event.
4. The history is kept in `localStorage` (`nso.watch.store`), one run per
   start, across reloads, and browsed in an in-app **Report** view - no
   download, no backend.
...
`npm run watch:test` (109 checks, real app in headless Chromium), in `npm test`.
...
**Paint display and commit attribution (2026-09-24, driver-testing round 2,
Seat + paint).** All four found by driving Seat with painted pieces; Seat's
own gap and paint-scope rules held throughout (0.180 mm by the checker; Seat
here stands down on a painted B that must turn, seats when only A is painted).
- `app-mask.js`: the yellow's position / rotation / quaternion / scale are
  the piece's own objects (`Object.defineProperties` in `buildOverlay`), so a
  drag, nudge, turn or Seat carries it - it used to stay on the plate where
  the piece was painted. And `sync()` (every frame, and in the bubble phase of
  every click / pointerup / keyup / change) repaints through
  `window.nsoMaskRepaint` whenever the active piece, its mesh or its geometry
  changed since the last repaint - the ~40 direct `editId` /
  `selectedIndex` writes (Join's slot picks among them) and bakes that swap
  geometry used to leave the previous yellow up. This retires watch:test B2
  (Seal / turn on a painted piece) as FIXED, per that file's own rule.
- `app-mask.js` status lines name the piece: `Paint cleared on "<name>"`
  (inside a Join, Clear acts on the ACTIVE piece A even while B is
  highlighted, and now says so when B's paint is left alone), and a paint
  click on a piece that is not the active one says the yellow is not drawn
  for it instead of "(0 tris yellow)".
- `nso-watch.js`: a commit is logged against the piece its undo entry names
  (`modelId`, or the plate slot `index`), with `- not the selected piece` /
  `- not the active piece` when that differs; `detail.target` carries it. A
  press on the 3D view is its own operation label (`press on the 3D view`),
  so paint / Seat here / drag commits no longer carry the last button.
`npm run watch:test` part G drives all of it; every new check fails on the
code before this change.

**Panel placement and collapse (2026-09-24, driver-testing round 1).** At a
1024 x 768 pane the expanded log covered the Undo button and the Scale card's
"Scale by" row, so a person with Watch on could not click them. The panel now
starts at `left: 64px`, clear of the left tool rail (Undo and the eye stay
clickable even expanded), and the header has a collapse button (▾ / ▸) that
shrinks it to the header bar - phantom / error pills still visible, logging
unchanged. `NSOWatch.collapse(bool)` / `isCollapsed()`; the choice is a
per-viewer preference in its own localStorage key, `nso.watch.collapsed`.
`npm run watch:test` part F.

## Cache-tag batch (2026-09-26)
Done on the owner's explicit instruction, as one pass after Crop Stage 2
(PR #114) and the multi-run union (PR #113) merged. This is an exception to
rule 4; the HUD string (`stampHud()`) is untouched.

| file | was | now |
| --- | --- | --- |
| `app-core.js` | `gsup2` | `gsup3` |
| `app-crop.js` | `crop3` | `crop4` |
| `nso_hollow.js` | `hollow3` | `hollow4` |
| `styles.css` | `legendmin1` | `crop4` |
| `app-microscope.js` | `scope9` | `scope10` |
| `nso-gcode-solid.js` | untagged | `solid1` |

The "needs a bump at merge" notes in the Crop Stage 2 and fused-selections
entries below are covered by this pass.

**Follow-up, same day: five older drifts the batch missed.** Checked by
content: a file has drifted when it differs at HEAD from the commit that set
its current tag (`git log -S "file?v=tag" -- index.html | tail -1`, then
`git diff --quiet <that> HEAD -- file`). The batch above covered the files
named in "needs a bump" notes; these five changed after their tags with no
note:

| file | was | now | changed since by |
| --- | --- | --- | --- |
| `nso_support_aim.js` | `support1` | `support2` | `ad22e9a` |
| `nso_stock.js` | `stock2` | `stock4` | `0b3f572` (`stock3` was used once before, so skipped) |
| `nso_path_sweep.js` | `taper1` | `taper2` | `2aa07a2` |
| `nso_support_tree.js` | `tree4` | `tree5` | `548a704`, `e50eaea` |
| `app-oversize-washer.js` | `oversize1` | `oversize2` | `f1aeac3` |

After this, no tagged file differs from its tag's content. Five scripts still
load untagged (`nso-cooling-profiles.js`, `nso-3mf.js`, `nso-3mf-read.js`,
`nso-gcode-lines.js`, `nso-watch.js`); they were untagged before and are
left so.

## Crop, stage 2 - move a region, reconnect it by what is measured (2026-09-26)

Branch `claude/crop-stage2-move`, off `claude-wip`. Full write-up:
`docs/CROP.md`, "Stage 2".

- **What.** The Crop box now selects a region (Up / Across narrow it across
  the section). Lift shows it as a ghost; drag it (camera-facing plane) or
  type ΔX ΔY ΔZ; **Lock in** reconnects it. The branch is MEASURED at lock-in:
  overlap → trim (`NSO_unionSoups`), touch → the same union, gap ≤ 3 mm →
  bridge (a closing of the two parts on Hollow's surface nets), else refused
  by name: gap too wide, region > 90 % buried, piece left in more parts, or
  the checker gate.
- **Where the kernel runs.** A slab along the cut axis only: cut out with
  `rawCut`, reconnected, spliced back with Stage 1's `NSO_cropSeal`. Outside
  it every vertex is Cut's clipped half's.
- **The seam-cap class, again, and how it is kept out.** A union that crosses
  the Mirror-Join's sliver fans leaves sub-1e-4 vertices (v9: non-manifold
  2 → 78). The lifted region is re-meshed by `Manifold.simplify` before it
  moves, and the reconnected slab is welded at Cut's radius. `simplify` on
  the SLAB is out: it collapsed a cap-outline vertex and broke the splice.
- **Acceptance** (`crop:move` §3-4): the v9 −Y lip, X 74..86 next to the
  +seam, to the +Y side at the same X. Own height → bridged (0.897 mm gap);
  2 mm down → trimmed (11.54 mm³). The box face exactly on the +seam ring,
  and the same span at the −seam, also pass. Every result PASSes
  mesh_validate.py with self-intersection, and has 0 defects near either
  seam ring. The source has 2 nm / 5 inc there. Split through the seam ring
  and the landing stays clean. Too extreme → refused: 6.90 mm gap, 96 %
  buried, > 10 mm.
- **Reuse.** `nso_hollow.js` exports `edt3d`, `surfaceNets`, `tidyPositive`.
  `NSO_Repair.inspect` gates the headless core too, not only the app.
- Checks: new `crop:move`, `wire:crop-move`.
- **`?v=`**: bumped in the cache-tag batch above (PR #115).
- **Not yet:** a region crossing BOTH seams (the next stress test), rotation,
  a region at the very end (Stage 1's axis clamp), and mixed contact (overlap
  here, gap there) in one move.

## G-scope - fused selections: one solid where runs touch (2026-09-26)
Branch `claude/gscope-multirun-union`, off `claude-wip`. Full write-up:
**`docs/GCODE-MICROSCOPE.md`, "G-scope: fused selections"**.

- **Closes** the solid export's stated limitation: a multi-run selection
  exported as touching shells. **Export as solid** and **Solid STL** now call
  `NSOGcodeSolid.fuseShells`. It finds the real junctions (stacked or side by
  side, `shellContacts`) and unions each connected group in `NSO_CSG`, pair by
  pair and only across a contact. A run touching nothing passes through bit
  for bit. The live Solid view stays unfused.
- **Kernel findings, all measured:** a float32 step between stacked planes
  reads as "no contact", so it is snapped. Zero-volume slivers appear between
  compound bodies; they are dropped. Negative-volume components are enclosed
  cavities; they are kept. The binding's `Manifold.compose` is a union and
  fills cavities. Each group is written to float32 once and then goes through
  the tree's `_finishUnion`.
- **Gated:** every fused body must pass `NSO_Defects.locate` (the checker's
  browser copy). The ways tried are exact → seated 1e-3 mm → stepwise. Beyond
  that, the beads at the located defects are set aside as swept. There is a
  guard at 4,000 runs / 20,000 junctions.
- **Checks:** `gcode:solid` §5 (62 in all) and `gcode:solid-drive` (44). Inner
  + Outer wall PASSES `mesh_validate.py` as one export. Its volume matches an
  independent scanline union to rel 1.2e-7. A single run is unchanged and
  takes 0-2 ms.
- **`?v=` not bumped** (rule 4): `app-microscope.js` and `nso-gcode-solid.js`
  need a bump at merge.

## Open ticket - G-scope renders on demand, not every frame (added 2026-09-26)
Open, not started. Found while fixing `gcode:solid-drive` (PR #110).

**The cost.** `animate()` in `app-microscope.js` re-queues itself with
`requestAnimationFrame` and calls `renderer.render` every frame for as long as
G-scope is open, whether or not anything changed. In headless Chromium
(software GL) the full `tabletop` slice costs ~2.5 s a frame, back to back:
measured with long-task timing, a real click on Export as solid took 23.4 s
and Read the plate 20.6 s on a quiet box, and past 30 s at 2x CPU throttle.
Real users on a weak or integrated GPU pay the same loop in fan, battery and
input lag, just at a smaller number.

**The ticket.** Draw only when something changed: a `needsRender` flag set by
the OrbitControls `change` event, `resize`, `viewChanged` / the detail and
highlight rebuilds, box drags and the layer slider, and cleared after one
render. Things that currently lean on the free-running loop and must still
work:
- `followClip()` and the corner dot's `cornerPoint()` run every frame; they
  must run on camera change instead.
- OrbitControls damping (if enabled) needs frames until it settles.
- `flushView()` and the screenshot checks assume the canvas is current; they
  must force one render.

**Done when:** G-scope sits idle with no long tasks while untouched, every
`gcode:*` G-scope check still passes, and `heavyClick()` in
`gscope-solid-drive-check.mjs` can go back to a plain `page.click` and still
pass at 2x throttle. That last one is the proof.

## G-scope supports - overhang markers, point-click generation, per-support edit (2026-09-25)
Branch `claude/overhang-point-support`, off `claude-wip` after PR #99. Full
write-up: **`docs/GCODE-SUPPORTS.md`**.

- **Markers** are auto-aim's own `detectOverhangs` regions: the region's own
  triangles tinted, and a ping at `region.point`. Red means unsupported, green
  means a support made for it still touches it where it is now.
- **Click a marker** to choose Straight strut / Tree / Gecko-hand and a size.
  Generate calls `NSO_SupportAim.plan` or `NSO_SupportTree.planTree` +
  `unionParts` with the marker's own inputs. No new geometry code.
- **Each support is its own plate piece**, placed exactly: the placement is
  read back off the scene and compared value for value. It is one `addModels`
  step.
- **Click a support** to select it through G-scope's seam. Apply (regenerate)
  or Remove is one step each, under the new `supportSwap` undo type in
  `app-core.js`, which restores both the model and its exact pose.
- **Paint scope:** detection is sub-region (`skipList`, shifted into the world
  frame, and refused by name on a piece that is painted and turned); editing a
  support is whole-piece, on the support.
- **Checks:** `gcode:gscope-support` (52). Markers are checked against the
  direct detection call; generated supports against the direct generator calls,
  value for value; strut, tree and gecko-hand through the canonical checker at
  the tree check's derived weld; Undo on add, change and remove. Plus
  `paint:scope` (new roster row). Ported onto `claude-wip` as
  `claude/overhang-point-support-port`; `?v=` tags at `scope8` / `gsup2`
  (`scope8` since PR #109: the #108 merge had loaded `app-microscope.js`
  twice - check for exactly one tag after any conflict in `index.html`).

### Driver round 3 (G-scope x Mirror) - three fixes (2026-09-25)
1. **Tree union regression (round sections).** Since the round-section /
   hollow-bore / contact-interface merges, branches are 24-gon frusta, and
   near the tips two of them cross at grazing angles. manifold returns the
   intersection as float32, which (a) collapses distinct intersection
   vertices onto one - triangles with a repeated corner, read by exact
   census as 2 degenerate + 2 non-manifold + 1 mis-wound edge on the shelf
   tree - and (b) leaves runs of vertices 1e-6 .. 6e-5 mm apart along the
   curve, which the canonical checker's default weld (1e-4, first-
   representative, not transitive) splits into open edges. The square
   section on the same plan is clean; the tree suites passed because none of
   their fixtures cross round branches that closely. Fix, in
   `nso_support_tree.js` `unionParts`, on the finished fold only
   (`_finishUnion`, exported for `tree:test`): drop the repeated-corner
   triangles, then collapse every edge shorter than 1e-4 mm onto one end
   (link condition, no flipped triangle), each step gated on the exact census
   and the signed volume (|dV| <= 1e-6 V). A weld at 1e-4 was tried first and
   is wrong: it also merges non-adjacent vertices across the crossing and
   reports non-manifold edges. Volume unchanged; gecko-hand 11050 -> 10918
   tris, PASS at the derived weld with Euler 2, and 0 open / 0 degenerate at
   the default weld.
2. **Stale green.** A marker was green whenever a support with that region
   id existed - mirror, move or turn the host and it stayed green over a
   region the support no longer touches. Now `placeSupport` records the
   support's contact vertices on the region (`meta.contacts`), and a region
   is green only if a labelled support's contacts ALL still touch its
   flagged triangles where they are now (signed distance -max(1, side) ..
   +0.02 mm along the face normal, within 0.02 mm laterally). Otherwise it is
   red, and the marker, popover and panel name the stale support.
   `gcode:gscope-support` section 6: host nudged / mirrored / yawed, support
   moved alone, each red then green on Undo. A support saved without
   `contacts` reads red.
3. **Resting height on Undo.** Ingest drops a piece at `+0.3`; every pose or
   move re-seats it on the `0.2` baseline. Undo re-derived y, so a piece came
   back 0.1 mm lower than it was. `posePlaced` / `movePlaced` entries now
   carry `meshY` and Undo restores it exactly (`restoreMeshY`). The 0.3 / 0.2
   split itself is left alone (about 20 sites); logged here.

## G-scope - one G-code tool, with line isolation, a layer slider and Crop (2026-09-25)
Branch `claude/gscope-consolidate`, off `claude-wip`. Full write-up:
**`docs/GCODE-MICROSCOPE.md`, "G-scope: one tool"** (it supersedes the
"toolpath import - the entry point" and "two rail buttons" entries below).

- **One tool.** `#btn-gcode` (the G🔬 door that only ever opened a file dialog,
  put a slice on the plate as feature models and never opened the view),
  `#gcode-input`, the sidebar Toolpath card and `app-toolpath.js` are removed.
  The rail button is `#btn-microscope`, now G🔬 "G-scope". A sliced project
  through Import / drop is recognised **by content** (a renamed slice used to
  fail with "No objects found") and opens in G-scope with **nothing added to
  the plate**.
- **View only:** Clear from view / Show all, the legend's "only", Select shown,
  the slider - none writes `state`, the plate or undo.
- **Line isolation:** Pick a line (click, Shift-click, ◀ ▶ / arrow keys);
  Isolate now draws `extrudeFlatLine` prisms - the exported solid - instead of
  the reference tool's cylinders.
- **Layer-range slider** docked right: one layer, a chunk, everything.
- **Crop** the slider's Z band through `app-crop.js`: a plate piece in place
  (`NSO_cropModel`, factored out of the Cut-menu Crop); a slice's selection as
  a new piece, via `NSO_cropRaw`'s new `opts.shells` gate for touching bead
  shells.
- **Seam** for support generation: `setSelection` / `selectionRegion()` /
  `onSelection()` / `setSelectionMoves()` - described, not built.
- Checks: new `gcode:gscope` (91, screenshots in `tools/gscope-out/`);
  `gcode:drive` and `gcode:entry` retired with the door they drove;
  `gcode:scope-drive`, `gcode:scope-overlay`, `crop:test` (§7) and the topbar
  test updated. **`?v=` not bumped - needs a bump at merge** for
  `app-microscope.js`, `app-crop.js`, `app-core.js`.


## Solidify - a progress gate, and two loops it could not close (2026-09-24)

Found by driver-testing round 1 (single operation, three sources of truth:
`nso_damage.js`'s prediction, the Watch log, `mesh_validate.py` on the result).
All three agreed - and what they agreed on was a bug: on
`fixtures/damage/shape-cylinder_holes-light.stl` (`hole`, shape-cylinder,
severity light, seed 7; 16 open edges in 2 loops) Solidify said
**"Solidify ok - open 16→16"**, pushed a `solidifyReplace` Undo, and left the
piece byte-identical. The result gate only asked "is it worse?".

**Now** (`app-finish.js`, comment above `solidifyCensus` and above
`SOLIDIFY_LOOP_MAX_EDGES`):
- **Progress gate.** A result with as many open edges as the input is
  `Solidify refused - closed none of the N open edges ... - left open: <why>;
  piece unchanged`, nothing committed, no Undo. Fewer but not zero commits as
  `Solidify partial` and names what was left and why. `Solidify ok` means 0 open.
- **Pinched loop** (two cap-fan holes meeting at the centre vertex: one
  boundary component with a degree-4 vertex). `capSmallOpenLoops` walks the
  boundary undirected and skips any loop with a degree != 2 vertex.
  `solidifyDirectedLoops` walks directed half-edges and splits the figure
  eight into its two simple cycles.
- **Folded loop** (three top-cap faces plus the wall face under them: folds
  90° over the rim and reaches z = 0, on no plane). `capAllOuterHoles` only
  caps loops on a bounding-box plane. `solidifyFillLoop` triangulates from the
  loop's own vertices with Liepa's weight (worst dihedral first - against the
  surrounding faces too - then area), which puts the crease back on the rim.
  A flat best-fit-plane fill was tried first: checker-clean, but it chorded
  through the solid and lost 1.04% of the cylinder's volume. With the dihedral
  weight the result's volume equals the clean cylinder's to 1e-13.
- Loops over 64 edges (NSO_Repair's `MAX_HOLE_EDGES`), open chains, and fills
  that must fold past 150° are left open and named in the status.
- **Winding joins the result gate.** Found while checking the new `partial`
  path: on `shape-sphere_holes-shredded.stl` the coarse welds +
  `repairJoinedSoup` close 13 of 221 open edges and mis-wind 21 (0 → 21 by the
  canonical checker). Open went down, nothing the gate watched went up, so it
  would have committed as progress. `solidifyRegressions` now also refuses on
  `mis-wound a→b` (NSO_Repair.inspect `flippedEdges`).

`tools/nso_wire_solidify_test.js` sections 4 (this fixture: both failure modes
shown present, then closed, PASS, 0 piercing, clean volume, one Undo), 5 (a
tilted open tube with two 80-edge rims: refused, bit-identical, no Undo) and 6
(the shredded sphere: refused on winding, bit-identical, no Undo).

Not changed, and still the open "coarse-weld tradeoff": the 0.2 / 0.3 mm grid
welds run first. On `shape-sphere_hole-painted.stl` they still raise NM and
the result gate refuses, even though the loop closer alone would close it.
Same story on the shredded sphere: the loop closer run on the input directly
takes it 221 → 85 open with 0 mis-wound; behind the welds it gets 221 → 208
with 21 mis-wound. That is the evidence for the coarse-weld ticket, not a
change made here.

**Follow-up, minor, not fixed here:** a plain click that selects a piece
(no drag) writes **"Moved model #N"**. `endMoveDrag()` in `app-core.js`
correctly skips `pushUndo` when nothing moved, but still calls
`setStatus('Moved model #' + (idx + 1))` unconditionally. Seen in the Watch log
of driver-testing round 1. Low priority; the log is honest, the app's line is not.
